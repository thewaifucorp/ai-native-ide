...
import { hostClient, type BenchmarkPreviewStatus, type HostCall, type PreviewFailureReport, type SemanticProjectRecord, type StartedAgentSession, type TerminalRunStatus, type WorkspaceResource } from "./host-client";
import "./styles.css";
...
function App() {
  ...
  const [effectState, setEffectState] = useState<"idle" | "awaiting" | "written" | "failed">("idle");
  const [agentSession, setAgentSession] = useState<StartedAgentSession | null>(null);
  const [agentCall, setAgentCall] = useState<HostCall<StartedAgentSession> | null>(null);
  const [terminal, setTerminal] = useState<TerminalRunStatus | null>(null);
  const [terminalCall, setTerminalCall] = useState<HostCall<TerminalRunStatus> | null>(null);
  ...
  async function capturePreviewFailure() {
    ...
    const result = await hostClient.stopAndCaptureBenchmarkPreviewFailure(project.id, resource.id, "benchmark-plan-v1");
    if (result.state === "available") {
      setPreview(null);
  ...
  async function approveBenchmarkPlan() {
    if (!project || !resource) return;
    const approved = await hostClient.approveNextWorkspaceWrite(project.id, resource.id);
  ...
  async function startAgentSession() {
    if (!project || !resource) return;
    const result = await hostClient.startReadOnlyAgentSession("claude", project.id, resource.id);
    setAgentCall(result);
  ...
  async function startWorkspaceInspection() {
    if (!project || !resource) return;
    const result = await hostClient.startWorkspaceInspection(project.id, resource.id);
  ...
  return <div className="app-shell">
    ...
    <main className="work-surface"><header className="surface-bar"><div><span className="eyebrow">{section.toUpperCase()} / {depth.toUpperCase()}</span><span className="connection"><i /> {project ? "Projeto local persistido · sem agente conectado" : "Local-first · sem agente conectado"}</span></div><div className="depth-switch" aria-label="Profundidade da interface"><button className={depth === "essential" ? "depth-button depth-button--active" : "depth-button"} onClick={() => setDepth("essential")} type="button">Essential</button><button className={depth === "raw" ? "depth-button depth-button--active" : "depth-button"} onClick={() => setDepth("raw")} type="button">Raw</button></div></header>
      <div className="surface-scroll"><section className="intent-hero" aria-labelledby="intent-heading"><div className="section-label"><span>01</span><span>INTENÇÃO</span></div><h2 id="intent-heading">O que você quer colocar no mundo?</h2><p className="lede">Comece pela mudança que você quer causar. A estrutura técnica pode esperar.</p><label className="intent-input-label" htmlFor="intent">Intenção do projeto</label><textarea id="intent" value={intent} onChange={(event) => setIntent(event.target.value)} placeholder="Ex.: quero criar uma ferramenta para…" rows={3} /><div className="intent-footer"><span>{projectCall?.state === "failed" ? `O host recusou o projeto: ${projectCall.message}` : projectCall?.state === "unavailable" ? "Abra o app desktop para persistir o projeto; nenhum projeto foi criado neste preview web." : project ? "Projeto persistido localmente; a intenção ainda não foi enviada a nenhum modelo." : intent.trim().length ? "Intenção local, ainda não enviada a nenhum modelo." : "Escreva livremente; vamos ajudar a tornar isso construível."}</span><button className="primary-action" onClick={() => void createProject()} type="button">{project ? "Atualizar projeto" : "Começar a construir"} <span>→</span></button></div></section>
        ...
        <section className="build-section" aria-labelledby="build-heading"><div className="section-label"><span>03</span><span>CONSTRUÇÃO</span></div><h2 id="build-heading">Um próximo passo, não uma parede de configuração.</h2><div className="next-step"><div><span className="eyebrow">PRÓXIMA DECISÃO</span><strong>{activeSignal?.title ?? nextStep}</strong><p>{activeSignal ? activeSignal.prompt : resource ? effectState === "awaiting" ? "O plano do benchmark está pausado para sua aprovação explícita." : effectState === "written" ? "O plano foi escrito no recurso anexado e está registrado como efeito da IDE." : "Prepare o plano do benchmark antes de iniciar o preview." : "Anexe um diretório para criar o primeiro artefato do benchmark."}</p></div><div>{effectState === "awaiting" ? <button className="primary-action" onClick={() => void approveBenchmarkPlan()} type="button">Aprovar plano <span>→</span></button> : <button className="outline-action" disabled={!resource || effectState === "written"} onClick={() => void proposeBenchmarkPlan()} type="button">{effectState === "written" ? "Plano aplicado" : "Preparar benchmark"}</button>}</div></div><div className="preview-placeholder"><div className="preview-top"><span>PREVIEW</span><span className={preview ? "status-healthy" : previewFailure ? "status-unknown" : "status-unknown"}>{preview ? "● HEALTHY" : previewFailure ? "● BROKEN" : "○ NOT-RUN"}</span></div>{preview ? <><iframe className="benchmark-preview" src={preview.url} title="Preview do leilão de posições" /><button className="text-button" onClick={() => void capturePreviewFailure()} type="button">Verificar falha real <span>→</span></button></> : <div className="preview-body"><div className="preview-skeleton preview-skeleton--title" /><div className="preview-skeleton" /><div className="preview-skeleton preview-skeleton--short" /><p>{previewFailure ? `${previewFailure.failure.message} Artefato: ${previewFailure.failure.causalLinks.filePaths.join(", ")}. Divergência registrada: ${previewFailure.divergence.id}.` : previewCall?.state === "failed" ? `O host não iniciou o preview: ${previewCall.message}` : previewCall?.state === "unavailable" ? "O preview só inicia pelo app desktop; nenhum servidor foi iniciado nesta página web." : effectState !== "written" ? "Prepare e aprove o plano do benchmark para criar seu primeiro artefato controlado." : "O benchmark real aparece aqui quando o host iniciar o preview local."}</p><button className="outline-action" disabled={!project || effectState !== "written"} onClick={() => void startPreview()} type="button">{project ? "Iniciar benchmark local" : "Crie o projeto primeiro"}</button></div>}</div></section>
    ...
    <aside className="context-dock" aria-label="Contexto atual"><header><span className="eyebrow">CONTEXTO ATUAL</span><button className="quiet-icon" aria-label="Fixar contexto" type="button">⌁</button></header><section className="dock-section"><span className="dock-label">AGENTE</span><strong>{agentSession ? "Claude conectado · somente leitura" : "Não conectado"}</strong><p>{agentCall?.state === "failed" ? `O adapter recusou a sessão: ${agentCall.message}` : agentSession ? agentSession.policyNote : "Conecte um adapter só depois de escolher o recurso. A IDE mantém efeitos de escrita fora do agente."}</p><button className="text-button" disabled={!project || !resource} onClick={() => void (agentSession ? cancelAgentSession() : startAgentSession())} type="button">{agentSession ? "Encerrar sessão" : "Conectar Claude"} <span>→</span></button></section><section className="dock-section"><span className="dock-label">TERMINAL</span><strong>{terminal ? "Inspeção PTY em execução" : "Somente leitura"}</strong><p>{terminalCall?.state === "failed" ? `O host recusou o terminal: ${terminalCall.message}` : terminal ? terminal.detail : "Execute uma inspeção Git limitada ao recurso; comandos livres entram somente com a política de terminal da fase seguinte."}</p><button className="text-button" disabled={!project || !resource} onClick={() => void (terminal ? cancelWorkspaceInspection() : startWorkspaceInspection())} type="button">{terminal ? "Cancelar inspeção" : "Inspecionar recurso"} <span>→</span></button></section><section className="dock-section"><span className="dock-label">ESCOPO</span><strong>Projeto inteiro</strong><p>{resource ? `${resource.kind === "repository" ? "Repositório" : "Diretório"} anexado · escopo ativo` : "Anexe um diretório para habilitar terminal, agente e efeitos."}</p></section><section className="dock-section"><span className="dock-label">APLICADO AGORA</span><div className="guidance-empty"><span>◌</span><p>Nenhuma guidance ativa ainda. Uma decisão que você salvar aparecerá aqui com origem e escopo.</p></div></section><section className="dock-section dock-section--bottom"><span className="dock-label">EFFECTS</span><strong>{effectState === "awaiting" ? "1 aguardando aprovação" : effectState === "written" ? "1 efeito aplicado" : effectState === "failed" ? "Efeito recusado pelo host" : "0 propostos"}</strong><p>{effectState === "awaiting" ? "O payload exato só será escrito após a aprovação." : effectState === "written" ? "Snapshot e revisão causal foram registrados pelo host." : "Nenhum efeito é autorizado por esta interface."}</p></section></aside>
